  ...
  namespace explore {
    ...
    namespace details {
      ...
      void run(tchecker::parsing::system_declaration_t const & sysdecl,
               ...
               tchecker::log_t & log, bool eqOrSim)
      {
        ...
        // std::cout << "EXTRA2\n";
        // auto systems = model.system();
        // tchecker::ta::system_t system_p(systems.name());
        // edge_t ed = *(*(systems.edges().begin()));
        // std::cout << typeid(*(systems.locations().begin())).name() << "\n";



        // for(auto iter = systems.locations().begin() ; iter != systems.locations().end() ; iter++){
          // bool com = (**iter).committed();
          // auto l = **iter;
          // bool init = l.initial();
          // tchecker::expression_t *expr = l.invariant_();
          // string pname = l.pid();
          // std::cout << typeid(l).name() << "\n";
          // bool init = l->initial();
          // bool initial = l.initial();
          // bool urgent = l.urgent();
          // system_p.add_location(l);
          // std::cout << (**iter).pid() << " " << (**iter).id() << " " << (**iter).name() << "\n";
        // }

        // for(auto iter = system_p.locations().begin() ; iter != system_p.locations().end() ; iter++){
        //   std::cout << (**iter).pid() << " " << (**iter).id() << " " << (**iter).name() << "\n";
        // }
        ...
        try {
          // std::cout << "RUN START\n";
          algorithm.run(ts, graph, eqOrSim);
        }
        catch (...) {
          graph.free_all();
          throw;
               ...
               tchecker::log_t & log, bool eqOrSim)
      {
        switch (options.output_format()) {
          case tchecker::explore::options_t::DOT:
            tchecker::explore::details::run<tchecker::graph::dot_outputter_t, WAITING>(sysdecl, options, log, eqOrSim);
            break;
          case tchecker::explore::options_t::RAW:
            tchecker::explore::details::run<tchecker::graph::raw_outputter_t, WAITING>(sysdecl, options, log, eqOrSim);
            break;
          default:
